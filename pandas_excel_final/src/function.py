...
def operate_exam(df_sheet, index_axis, student_id):
    if index_axis == 'nan':
        logger.error('未找到班级信息，程序终止')
        print('未找到班级信息，程序终止')
        exit()
    else:
        # 取参数部分
        if '总分' in df_sheet.columns:
            df_sheet = df_sheet.loc[:, :'总分']
            df_sheet = df_sheet.drop(columns='总分', axis=1)
        # df_sheet = df_sheet.drop(columns='总分', axis=1)
        ...
        df_score = df_score.drop(columns=['学生学号', '班级', '学生姓名'])
...
def get_object_sheet(df_test_weight_index, sheet_name, sheet):
    ...
    for i in df_test_weight_index:
        ...
        if '总分' in df_test_sheet.columns:
            df_test_sheet = df_test_sheet.loc[:, :'总分']
            df_test_sheet = df_test_sheet.drop(columns='总分', axis=1)
        object_evaluation_sheet.append(df_test_sheet)
    # print('页帧名称：', sheet_name)
    return object_evaluation_sheet

@logger.catch
# 获取主观评价表格 教师评价法, 学生自评法
...
def operate_object_sheet(object_evaluation, df_test_weight, student_id, class_sheet, df_param, df_score):
    ...
    df_target_check = copy.copy(df_test_weight)
    df_target_check.index = range(len(df_test_weight))
    ...
    for i in range(len(object_evaluation)):
        if i != class_sheet:
            ...
            # 学生成绩部分
            columns_name = object_data.loc['学生学号', :]
            # object_data.drop(columns='学生姓名', inplace=True)
            object_scores = object_data.loc[student_id]
            object_scores = object_scores.dropna(axis=1, how='all')
            object_scores.columns = columns_name
            object_scores.drop(columns='学生姓名', inplace=True)
            try:
                object_data.columns = object_data.loc['参数', :]
                object_data = object_data.drop(index='参数')
                ...
                # 检查目标个数是否一致
                k += 1
                check_data.check_target_num(object_parameter, df_test_weight, df_test_weight.index.values[k], df_target_check.loc[i], sheet_name[i])
                # 检查小题分数满分是否符合要求
                # check_data.check_out(object_parameter, df_test_weight.index.values[k])
                # print(object_parameter)
            ...
            # 检查得分是否超过总分
            check_data.check_out_error(object_parameter, object_scores, sheet_name[i])
        else:
            k += 1
            object_all_param.append(df_param)
            object_all_scores.append(df_score)
            # 检查目标个数是否一致
            check_data.check_target_num(df_param, df_test_weight, df_test_weight.index.values[k], df_target_check.loc[i], sheet_name[i])
            # 检查得分是否超过总分
            check_data.check_out_error(df_param, df_score, sheet_name[i])
            # print(object_all_param)
            # 检查小题分数满分是否符合要求
...
def operate_unobject_sheet(unobject_sheet, unobject_columns, student_id, df_index_values):
    ...
    try:
        for i in unobject_sheet:
            ...
            # 分数部分
            i.columns = i.loc['学生学号', :]
            single_unobject_scores = i.loc[student_id]
            single_unobject_scores.drop(columns='学生姓名', inplace=True)
            single_unobject_scores.dropna(axis=1, how='all', inplace=True)
            single_unobject_scores.columns = unobject_columns

            # 检查得分是否超过总分
            check_data.check_out_error(single_param, single_unobject_scores, df_index_values[k])
...
def count_object_evaluation(single_param, single_scores, df_test_weight):
    ...
    count_param = single_param.drop(index='小题分数', axis=0)
    # 计算各目标得分
...
def count_class_unobject(unobject_scores, unobject_param, everyclass_student, class_achieve, class_average):
    try:
        for i in everyclass_student:
            ...
            average = round(scores.mean(axis=0), 3)
            unobject_param.columns = unobject_scores.columns
            achieve = average.div(unobject_param)
            achieve = achieve.astype(float)
            achieve = round(achieve, 3)
...
def static_data(achieve, student_id, df_weight):
    ...
    for j in achieve_score.index.values:
        ...
        if count == len(achieve.columns):
            count4 += 1
    odd4 = ('%.3f%%') % ((count4 / len(student_id)) * 100)
    ...
    odd5 = ('%.3f%%') % ((count5 / len(student_id)) * 100)
    ...
    odd6 = ('%.3f%%') % ((count6 / len(student_id)) * 100)
