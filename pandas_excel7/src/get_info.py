...
def get_data(sheet):
    ...
    try:
        ...
        # df_test_weight = df_test_weight.dropna(axis=1, how='all')
        # df_test_weight = df_test_weight.fillna(value=0)
        # 检查权重和是否为1
...
def operate_weight(df_test_weight, data_weight):
    ...
    df_weight = df_weight * data_weight['考核成绩法']
    data_weight = data_weight.drop(index='考核成绩法')
...
def operate_sheet(df_test_weight, sheet, df_index_values, df_weight, data_weight):
    ...
    return final_target_achieve, new_path, student_id, k, object_all_out
