...
def get_avg(option):
    ...
    try:
        for i in df_weight.columns:
            df_sheet = pd.read_excel(file, sheet_name=str(i), header=None)
...
def get_every_scores_achieve():
    ...
    try:
        ...
        for i in df_test_weight.index:
            ...
            df_sheet = pd.read_excel(file, sheet_name=str(i))
...
def get_final_achieve():
    ...
    try:
        # excel数据
        excel_achieve = pd.read_excel(file, sheet_name='达成度散点图', header=2, index_col=0)
...
def get_static_data():
    try:
        ...
        # excel数据
        df_sheet = pd.read_excel(file, sheet_name='达成度散点图', header=2, index_col=0)
