...
def write_out_achieve(wb, object_all_params, object_all_scores, object_evaluation_achieve, object_evaluation_out,
    ...
    try:
        ...
        for i in range(len(sheet_name)):
            df_old = pd.read_excel(sheet, sheet_name=str(sheet_name[i]), header=None)
            ...
            # 学生总分、得分、达成度
            end_col = student_data_to_excel(sht, df_old, object_all_scores[i], col_nums + 1, object_evaluation_out[i], everyclass_student, sheet_name[i], param_out, object_evaluation_achieve[i], df_test_weight)
            # 将写入数据部分设置居中和框线
...
def param_to_excel(sht, df_test_weight, object_param, col_nums, i, k):
    ...
    sht[1, col_nums].value = '总分'
    if i != k:
        sht[3, col_nums].options(transpose=True).value = object_out.values.tolist()
    else:
        sht[4, col_nums].options(transpose=True).value = object_out.values.tolist()
    return object_out.T
...
def object_class_data(df_sheet, class_ach, class_avg, everyclass_student, col_nums, class_id, sht):
    for i in range(len(everyclass_student)):
        ...
        sht[index_id + 1: index_id + 3, col_nums - 1: col_nums + len(average.columns)].api.Borders.LineStyle = 1      # 框线

# 学生总分、得分、达成度
def student_data_to_excel(sht, df_old, object_scores, col_nums, object_evaluation_out, everyclass_student, sheet_name, param_out, object_evaluation_achieve, df_test_weight):
    # 得分参数部分
    ...
    # 参数部分样式
    sht[1: 6 + len(df_test_weight.columns), col_nums-1: col_nums + len(param_out) * 2].api.Borders.LineStyle = 1     # 框线
    sht[1: 6 + len(df_test_weight.columns), col_nums-1: col_nums + len(param_out) * 2].color = (242, 242, 242)       # 背景颜色
    ...
    return col_nums + len(all_data.columns) - 1

# 客观评价表格样式
def excel_style(sht, col_nums, end_col):
    info = sht.used_range
    nrows = info.last_cell.row          # 有效数据行数
    sht[1:nrows, col_nums: end_col].api.HorizontalAlignment = -4108                     # 居中
    sht[1:nrows, col_nums: end_col].api.Font.Size = 10                                  # 字体大小
...
def write_reporter(wb, sheet, df_test_weight, class_ach, class_index, df_index_values, data_weight):
    try:
        ...
        for i in range(len(df_test_weight.index) + 1):
            sht.api.Rows(row + 2).Insert()
        sht[row+1, 1].value = data.T
        sht[row+1: row+2+len(df_test_weight.index), 2: 2+len(df_test_weight.columns)].api.Font.Name = '宋体'
        # 合并单元格
        for i in range(len(df_test_weight.index)+1):
            sht[row + 1 + i, 0:2].api.Merge()
        sht[row + 1, 0].value = '考核方式'
        # 处理课程目标达成度情况
        weight = list()
